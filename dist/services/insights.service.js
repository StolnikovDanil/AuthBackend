...
const targetIdFor = (attempt) => {
    return attempt.userId !== null ? `user-${attempt.userId}` : hashEmail(attempt.email);
};
const computeTopFailStreaks = (attemptsAsc, limit = 5) => {
...
const countNewDeviceEvents = async (periodAttempts, periodStart) => {
    const successfulWithUser = periodAttempts.filter((a) => a.success && a.userId !== null);
    if (successfulWithUser.length === 0)
        return 0;
    const userIds = [...new Set(successfulWithUser.map((a) => a.userId))];
    ...
    for (const attempt of successfulWithUser) {
        ...
        if (ipIsNew || userAgentIsNew)
            newDeviceCount++;
    }
    return newDeviceCount;
};
export const buildInsightsPayload = async (periodHours = DEFAULT_PERIOD_HOURS) => {
